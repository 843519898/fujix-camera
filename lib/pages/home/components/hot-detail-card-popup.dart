...
class HotDetailCardPopup extends StatelessWidget {
  final List<dynamic> videoList;
  const HotDetailCardPopup({
    Key? key,
  ...
  void _openInAppBrowser(video, context) async {
    if (video.containsKey('aweme_info')) {
      final String awemeIdOri = video['aweme_info']['aweme_id'].toString();
    ...
    return Container(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          video['aweme_info']?['cover'] as String?;
                ...
                children: [
                  ClipRRect(
                    ...
                    child: CachedNetworkImage(
                      imageUrl: (processedCoverUrl != null &&
                          processedCoverUrl.isNotEmpty)
                          ? processedCoverUrl
                ...
                child: GestureDetector(
                    onTap: () async {
                      ...
                      if (userInfo.containsKey('token') && userInfo['token'] != '') {
                        ...
                        //   '${AppEnv.h5BaseUrl}/h5/kj/loading?is_navi=0&aweme_id=${video['aweme_info']['aweme_id']}&aweme_type=1&clipScene=HotspotCut',
                        // ).then((success) {
                        //   if (!success) {
                        //   }
                        // });
